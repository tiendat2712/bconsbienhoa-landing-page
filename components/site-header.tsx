  ...
  return <header className="fixed inset-x-0 top-0 z-50">
    ...
    <div className={`transition-all duration-300 ${scrolled ? 'border-b border-border/70 bg-background/90 shadow-[0_8px_30px_-18px_rgba(16,185,129,0.35)] backdrop-blur-xl' : 'bg-background/65 backdrop-blur-md'}`}>
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 md:h-20 lg:px-8">
        <a href="#top" className="group flex shrink-0 items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-primary font-serif text-lg text-primary-foreground transition-transform group-hover:scale-105">B</span><span className="hidden flex-col leading-none sm:flex"><span className="font-serif text-lg text-foreground">Bcons</span><span className="text-[0.6rem] font-semibold tracking-[0.28em] text-muted-foreground uppercase">Central Park</span></span></a>
        <nav className="hidden items-center gap-1 xl:flex" aria-label={t.nav.main}>
          {navLinks.map(([label,href]) => <a key={href} href={href} className="group relative rounded-md px-2.5 py-2 text-sm font-medium text-foreground/80 transition-colors hover:text-primary">{label}<span className="absolute inset-x-3 -bottom-0.5 h-px origin-left scale-x-0 bg-primary transition-transform group-hover:scale-x-100" /></a>)}
          <div className="relative" onMouseEnter={() => setDetailOpen(true)} onMouseLeave={() => setDetailOpen(false)}>
            ...
            <div className={`absolute top-full left-0 w-56 pt-3 transition-all ${detailOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none -translate-y-1 opacity-0'}`}><div className="rounded-2xl border border-border bg-card/95 p-2 shadow-xl backdrop-blur-xl">{detailLinks.map(([label,href]) => <a key={href} href={href} className="block rounded-xl px-3 py-2.5 text-sm text-foreground/80 hover:bg-secondary hover:text-primary">{label}</a>)}</div></div>
          </div>
          <a href="#tin-tuc" className="rounded-md px-2.5 py-2 text-sm font-medium text-foreground/80 hover:text-primary">{t.nav.news}</a>
        </nav>
      ...
      <div className={`overflow-hidden border-t border-border/60 bg-background/95 backdrop-blur-xl transition-[max-height,opacity] duration-300 xl:hidden ${mobileOpen ? 'max-h-[36rem] opacity-100' : 'max-h-0 opacity-0'}`}><nav className="mx-auto grid max-w-7xl grid-cols-2 gap-1 px-4 py-4" aria-label={t.nav.mobile}>{[...navLinks,[t.nav.investor,'#chu-dau-tu'],[t.nav.progress,'#tien-do'],[t.nav.news,'#tin-tuc']].map(([label,href]) => <a key={href} href={href} onClick={() => setMobileOpen(false)} className="rounded-xl px-3 py-3 text-sm font-medium text-foreground/85 hover:bg-secondary hover:text-primary">{label}</a>)}<a href="#dang-ky" onClick={() => setMobileOpen(false)} className="col-span-2 mt-2 flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground"><Phone className="size-4" />{t.nav.consult}</a></nav></div>
    </div>
  </header>
}
